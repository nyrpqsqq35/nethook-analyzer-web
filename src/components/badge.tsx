...
export function Badge2({ color = 'zinc', className, ...props }: BadgeProps & React.ComponentPropsWithoutRef<'span'>) {
  return (
    <span
      {...props}
      className={clsx(
        className,
        'inline-flex items-center justify-center gap-x-1.5 rounded-md min-w-7 p-0.5 text-xs/5 font-medium forced-colors:outline',
        colors[color],
      )}
    />
  )
